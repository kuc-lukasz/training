...
// tworzenie typu dla każdej pojedyńczej karty
export type CardType = {
    id: string;
    //flip czy jest odwrócona
    flipped: boolean;
    backImage: string;
    frontImage: string;
    clickable: boolean;
...
// zapis typescript
// umieszczenie wszystkich kart w tablicy
const cards: string[] = [
    card1,
    card2,
    card3,
...
export const createBoard = (): CardType[] =>
    [...cards, ...cards].map((card, i) => ({
        ...
        frontImage: card,
        clickable: true,
        // stworzylismy drugi element gdzie bedzie pokazana druga karta z innym id do dopasowania
        matchingCardId:
            i < cards.length
                ? `card${i + cards.length}`
                : `card${i - cards.length}`,
